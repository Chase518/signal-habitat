...
## 2026-08-04　Python: sensor_metadata/sensor_reading/detection_event persisted, shared SQLite file across services

**Decision:** `analysis-python/app/storage.py` implements the three tables 项目总纲.md's schema always specified but that had never been built — Python regenerated all simulated data in memory on every request instead of storing it. `run_pipeline()` now generates once and persists; every later call (any process) loads the stored rows instead of re-simulating. `detection_event` stores only confirmed detections, not one row per reading. Python writes to the same SQLite file (`data/signal-habitat.db`) the Java backend already caches `analysis_result` in — each service only ever writes its own tables.

**Reasoning:** This mirrors the generate-once-reuse-many pattern already used for the Java-side `analysis_result` cache (see the 2026-08-03 Java backend entry), just applied one layer earlier — raw inputs instead of the computed output. Sharing one SQLite file rather than giving Python its own database keeps the "one project, one data store" story simple for a demo, and matches how 项目总纲.md always described the schema (four tables, not two databases); the risk this accepts is SQLite's file-level locking under genuinely concurrent writes from two different processes, which is a non-issue at this project's request volume. `sensor_reading` stores raw, pre-interpolation values (including gaps) rather than the interpolated series, because interpolation is an analysis-time concern, not a storage-time one — a real device wouldn't retroactively fill in a reading it never sent.

**Bug caught along the way:** loading detections back from storage could leave the aggregation step's merged `detected` column as object dtype (NaN mixed with Python `bool` after the left join), which made an all-absent temperature bin's `n_detections` serialize as the JSON literal `false` instead of `0` — silently wrong data, not a crash. Fixed by forcing real `bool` dtype before aggregating, verified with a regression test that explicitly rejects `bool` (Python's `bool` subclasses `int`, so a naive `isinstance(x, int)` check would not have caught this).
...
## 2026-08-04　Sensors/Detections features: Java reads the shared SQLite tables directly, no Python round-trip

**Decision:** `GET /api/sensors` and `GET /api/detections` follow the same hexagonal shape as `/api/analysis` (domain model, in/out ports, application service, adapters) but their persistence adapters (`SqliteSensorRepository`, `SqliteDetectionRepository`) query `sensor_metadata`/`sensor_reading`/`detection_event` directly via JDBC, rather than adding new Python endpoints and having Java proxy them the way `PythonAnalysisAdapter` proxies the regression. `SensorApplicationService`/`DetectionApplicationService` also skip the TTL-cache pattern `AnalysisApplicationService` uses.

**Reasoning:** Those three tables already live in the SQLite file Java has a JDBC connection to (see the 2026-08-04 Python persistence entry) specifically so each service can read what it needs without going through the other's API — building a Python `/api/sensors` just to re-expose data sitting in a file Java can already query would be pure plumbing with no benefit for a demo. No cache/TTL because these are cheap local SQLite reads, not an expensive Python computation; the caching pattern exists specifically to shield against repeatedly paying for regression fitting, and reusing it here for a query that costs microseconds would just be copying a pattern without its reason. `SqliteConnectionProvider` now also declares (but never writes) these three tables' schema, so Java doesn't 500 on a fresh checkout where it starts before Python has ever run once.
